...
export function agentArtifactFolder(scope: AgentDriveScope, agentId: string): { root: string; folderPath: string; folderHostPath: string; scopeKind: 'user' | 'organization'; scopeId: string; deletable: false } {
  const resolved = agentDriveRoot(scope);
  const folderPath = `/${resolved.scopeId}/agents/${clean(agentId)}/artifacts/FILES`;
  const folderHostPath = resolve(resolved.root, folderPath.slice(1));
  const base = resolve(resolved.root);
  if (folderHostPath !== base && !folderHostPath.startsWith(`${base}${sep}`)) throw new BadRequestError('Agent artifact folder escaped the mounted drive.');
  mkdirSync(folderHostPath, { recursive: true, mode: 0o700 });
  return { ...resolved, folderPath: `/drive${folderPath}`, folderHostPath, deletable: false };
}
...
export async function writeAgentArtifactFile(input: {
  scope: AgentDriveScope;
  agentId: string;
  fileName: string;
  mimeType?: string | null;
